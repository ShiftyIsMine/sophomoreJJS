...
public interface MemberRepository02 extends JpaRepository<Member, Integer> {
    ...
    public int updateMethod(@Param("e_name")String name, @Param("e_age")int age, @Param("e_email")String email,@Param("e+id")int id);

    @Transactional
    @Modifying
    @Query(value = "delete from Member where id = :e_id")
    public int deleteMethod(@Param("e+id")int id);
}
